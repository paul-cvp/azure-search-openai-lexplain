...
class Orchestrate(Approach):
    ...
    async def run(
        ...
        result = await self.run_once(messages, session_state, context)
        answer = result['message']['content']
        if is_xml(answer):
            dcr_graph = flatten_dcr(dcr_importer.deserialize(answer))
            current_app.logger.info(dcr_graph.events)
            semantics = DcrSemantics()
            enabled_events = semantics.enabled(dcr_graph)
            is_graph_accepting = semantics.is_accepting(dcr_graph)
            result['message']['content'] = f"enabled: {enabled_events} \n accepting {is_graph_accepting}"
        else:
            current_app.logger.info(result['context'])
        return result
