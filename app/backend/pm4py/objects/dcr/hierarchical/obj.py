...
class HierarchicalDcrGraph(ExtendedDcrGraph):
    ...
    def __init__(self, template=None):
        ...
        self.__nestedgroups = dict() if template is None else template['nestedgroups']
        self.__subprocesses = dict() if template is None else template['subprocesses']
        self.__nestedgroupsmap = dict() if template is None else template['nestedgroupsMap']
        if len(self.__nestedgroupsmap) == 0 and len(self.__nestedgroups) > 0:
            self.__nestedgroupsmap = dict()
    ...
    @nestedgroups.setter
    def nestedgroups(self, ng: Set[str]):
    ...
    def nestedgroupsmap(self, ngm: str):
    ...
    @subprocesses.setter
    def subprocesses(self, sps: Set[str]):
